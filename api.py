...
class ResNet(ImageClassificationBase):
    def __init__(self):
        super().__init__()
        self.network = models.resnet50(pretrained=True) # Use a pretrained model
...
def upload_predict():
    if request.method == "POST":
        ...
        if image_file:
            image_loc = os.path.join(FOLDER + 'static/img/', image_file.filename)
            image_file.save(image_loc)
            pred = predict_external_image(image_loc, MODEL)
            return render_template("index.html", prediction='My heart says that this is ' + pred + '.',img='static/img/'+image_file.filename)
        else:
